  ...
  return (
    <section className="group relative overflow-hidden border border-white/[0.08] bg-[#020202] p-5 shadow-[0_0_60px_rgba(0,255,136,0.035)]">
      ...
      <div className="flex items-start gap-3">
        ...
        <div>
          <p className="font-mono text-[10px] uppercase tracking-[0.22em] text-[#00FF88]">Registration choice</p>
          <h2 className="mt-2 text-2xl font-black uppercase leading-none">Continue registration in Telegram</h2>
          <p className="mt-2 text-sm leading-6 text-white/45">
            Use the guided bot flow for event confirmation, registration details, ticket reservation,
            and the payment step when it is connected.
          </p>
        </div>
      </div>
      <div className="mt-5 grid gap-2 border-y border-white/[0.05] py-3">
        <div className="flex min-h-12 items-center justify-between gap-3 border border-white/[0.05] bg-[#030303] px-4 text-sm text-white/60">
          ...
          <span className="font-mono text-[10px] uppercase tracking-[0.16em] text-white/30">Available</span>
        </div>
        <Link
          href={telegramUrl}
          target="_blank"
      ...
      {referralCode ? (
        <p className="mt-4 font-mono text-[10px] uppercase tracking-[0.18em] text-white/35">
          Referral applied: <span className="text-[#00FF88]">{referralCode}</span>
        </p>
      ) : null}
      <style
