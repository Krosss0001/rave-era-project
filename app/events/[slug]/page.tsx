  ...
  return (
    <div className="bg-[#000000]">
      <section className="group relative min-h-[calc(100svh-4rem)] overflow-hidden border-b border-white/[0.05] lg:min-h-[86vh]">
        ...
        <div className="relative mx-auto flex min-h-[calc(100svh-4rem)] max-w-7xl items-end px-4 pb-12 pt-28 sm:px-6 md:px-10 lg:min-h-[86vh] lg:px-12 2xl:max-w-[1500px]">
          <div className="w-full max-w-[58rem]">
            <div className="flex flex-wrap items-center gap-2">
              <span className="inline-flex min-h-8 items-center border border-[#00FF88]/40 bg-[#00FF88]/5 px-4 py-2 font-mono text-[10px] font-semibold uppercase tracking-[0.2em] text-[#00FF88]">
                {event.status === "limited" ? "Limited capacity" : event.status === "live" ? "Tickets live" : "Coming soon"}
              </span>
              {referralCode ? (
                <span className="inline-flex min-h-8 items-center border border-white/[0.05] bg-[#020202] px-4 py-2 font-mono text-[10px] font-semibold uppercase tracking-[0.2em] text-white/60">
                  Ref {referralCode}
                </span>
              ) : null}
            </div>
            <h1 className="drop-reveal mt-6 max-w-[16ch] whitespace-normal text-[clamp(2.5rem,13vw,4.75rem)] font-black uppercase leading-[0.9] tracking-normal text-white [hyphens:none] [overflow-wrap:break-word] [text-wrap:balance] [word-break:normal] sm:max-w-[18ch] sm:text-[clamp(3.5rem,9vw,6rem)] lg:max-w-[17ch] lg:text-[clamp(4.5rem,7.5vw,7.25rem)]">
              {event.title}
            </h1>
            <p className="drop-reveal mt-7 max-w-[68ch] text-base font-light leading-8 text-white/65 sm:text-lg">{event.subtitle}</p>
            <p className="mt-5 inline-flex max-w-full border-l border-[#00FF88] pl-4 font-mono text-[10px] font-semibold uppercase tracking-[0.16em] text-[#00FF88] motion-safe:animate-[signalPulse_1.8s_ease-out_infinite] sm:tracking-[0.22em]">
              {event.urgencyNote || "Last wave of tickets / Access closing soon"}
            </p>
            <LiveEventSignals eventId={event.id} />
            <div className="drop-reveal mt-10 grid gap-3 font-mono text-[10px] uppercase tracking-[0.14em] text-white/65 sm:grid-cols-2 lg:grid-cols-4">
              {[
                ...
                <div key={text as string} className="flex min-h-14 min-w-0 items-center gap-3 border border-white/[0.05] bg-[#020202]/85 px-4 py-3 backdrop-blur-sm motion-safe:transition-colors motion-safe:duration-500 hover:border-[#00FF88]/25">
                  <Icon className="h-4 w-4 text-[#00FF88]" aria-hidden="true" />
                  <span className="min-w-0 break-words">{text as string}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <section className="mx-auto grid max-w-7xl gap-10 px-4 py-20 sm:px-6 md:px-10 md:py-28 lg:grid-cols-[minmax(0,1fr)_360px] lg:gap-12 lg:px-12 xl:grid-cols-[minmax(0,1fr)_384px] 2xl:max-w-[1500px]">
        <div className="space-y-8 md:space-y-10">
          <article className="border-y border-white/[0.05] bg-[#020202] py-8 md:px-8 md:py-10">
            ...
            <h2 className="drop-reveal mt-5 max-w-3xl text-4xl font-black uppercase leading-[0.95] text-white md:text-6xl">
            ...
            <div className="mt-8 grid gap-3 sm:grid-cols-2">
              {[
                ...
                <div key={detail} className="min-w-0 break-words border border-white/[0.05] bg-[#030303] p-4 font-mono text-[10px] uppercase tracking-[0.16em] text-white/60 sm:tracking-[0.18em]">
                  {detail}
                </div>
              ))}
              {event.tags.map((tag) => (
                <div key={tag} className="min-w-0 break-words border border-white/[0.05] bg-[#030303] p-4 font-mono text-[10px] uppercase tracking-[0.16em] text-white/60 motion-safe:transition-colors motion-safe:duration-500 hover:border-[#00FF88]/30 hover:text-[#00FF88] sm:tracking-[0.18em]">
                  {tag}
                </div>
              ))}
            </div>
            ...
            <div className="mt-8 grid gap-3 sm:grid-cols-2">
              {event.lineup.map((artist, index) => (
                <div key={artist} className="group relative overflow-hidden border border-white/[0.05] bg-[#030303] p-5 motion-safe:transition-colors motion-safe:duration-500 hover:border-[#00FF88]/30 hover:bg-[#00FF88]/[0.02]">
                  ...
                  <p className="mt-3 break-words text-2xl font-black uppercase leading-none text-white motion-safe:transition-colors motion-safe:duration-500 group-hover:text-[#00FF88]">
                    {artist}
                  </p>
                </div>
              ))}
        ...
        <aside className="space-y-5 lg:sticky lg:top-24 lg:self-start">
          <section className="relative border border-[#00FF88]/20 bg-[#020202] p-5 shadow-[0_0_90px_rgba(0,255,136,0.07)]">
            ...
            <p className="mt-3 font-mono text-4xl font-semibold leading-none text-white"><LocalizedPrice price={event.price} currency={event.currency} /></p>
            {urgent ? (
            ...
            <div className="mt-5 grid gap-2 border-t border-white/[0.05] pt-4">
              {[
                ...
                <p key={item} className="flex min-w-0 items-center gap-2 font-mono text-[10px] uppercase tracking-[0.14em] text-white/45 sm:tracking-[0.16em]">
